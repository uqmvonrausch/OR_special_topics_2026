...
with open("./data/scenario_-1.json") as data_file:
...
m.setObjective(
    ...
    gp.quicksum((j.distance / 1000) * X[j,t] * (data["cost_loaded_per_m"] + data["fixed_cost_loaded_per_m"]) for j in Jobs for t in T), gp.GRB.MAXIMIZE)

m.optimize()

if m.Status == gp.GRB.OPTIMAL or m.Status == gp.GRB.SUBOPTIMAL:

    n_trucks = {t:round(sum(Y[s,t].x for s in S) + sum(Z[d,t].x for d in D)) for t in T}
